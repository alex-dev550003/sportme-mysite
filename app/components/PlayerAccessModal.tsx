...
export default function PlayerAccessModal({ onClose, playerWebUrl, playerPlayStoreUrl }: Props) {
  ...
  return (
    <div className="fixed inset-0 isolate z-[999] flex items-center justify-center bg-black/72 px-4 py-6" onClick={onClose} role="dialog" aria-modal="true">
      <div
        className="manager-access-modal relative z-[1000] w-full max-w-4xl rounded-[28px] border border-[#d8d1bf] bg-[radial-gradient(circle_at_top_right,_#ffffff_0%,_#f3f4f6_62%,_#eceff3_100%)] p-5 shadow-[0_30px_80px_-38px_rgba(32,33,31,0.9)]"
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div className="inline-flex items-center gap-3">
            ...
            <div>
              <p className="text-[19px] font-semibold leading-tight text-[#1f211f]">SportMe</p>
              <p className="text-sm text-[#5b564b]">{t("pricing.signupModal.subtitle")}</p>
            </div>
          </div>
          <button type="button" onClick={onClose} className="rounded-full border border-[#d8d1bf] bg-white px-2.5 py-1 text-xs font-semibold text-[#1f211f] hover:bg-[#f3f1e8]">
            {t("common.close")}
          </button>
        </div>
          ...
          <div>
            ...
            <div className="flex flex-col items-center gap-1.5">
              <button
                ...
                onClick={() => void openExternal(playerWebUrl)}
              >
                <WebIcon />
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="flex flex-col items-center gap-1.5">
                <button
                  type="button"
                  className="group inline-flex w-full items-center justify-center gap-2 rounded-full border border-[#d8d1bf] bg-white px-4 py-3 text-[#2a2d33] shadow-sm transition hover:-translate-y-0.5 hover:bg-[#f9fafb]"
                  onClick={() => void openExternal(playerPlayStoreUrl)}
                >
                  <GooglePlayIcon />
                  <span className="flex min-w-0 flex-col items-start leading-tight">
